...
\vspace{10pt}
\normalsize
\textbf{ANALFABETISMO Y EDUCACIÓN INDÍGENA EN MÉXICO:} \\
\textbf{EVALUACIÓN DE IMPACTO DEL PROGRAMA DE INCLUSIÓN Y EQUIDAD EDUCATIVA} \\
\vspace{20pt}
\Large
\textbf{T E S I S} \\
\vspace{5pt}
...
\vspace{5pt}
\textbf{MAESTRO EN ECONOMÍA APLICADA} \\
\vspace{5pt}
PRESENTA \\
\vspace{5pt}
\textbf{ALEJANDRO GRIMALDI FERREIRA} \\
\vspace{20pt}
ASESOR: DR. ARTURO ALBERTO AGUILAR ESTEVA \\
\vspace{20pt}
...
"Con fundamento en los artículos 21 y 27 de la Ley Federal del Derecho de Autor y como titular de los derechos moral y patrimonial de la obra titulada \textbf{"ANALFABETISMO Y EDUCACIÓN INDÍGENA EN MÉXICO: EVALUACIÓN DE IMPACTO DEL PROGRAMA DE INCLUSIÓN Y EQUIDAD EDUCATIVA"}, otorgo de manera gratuita y permanente al Instituto Tecnológico Autónomo de México y a la Biblioteca Raúl Bailleres Jr. autorización para que fijen la obra en cualquier medio, incluido el electrónico y la divulgación entre sus usuarios, profesores, estudiantes o terceras personas, sin que pueda percibir por la divulgación una contraprestación".
...
Gracias a mis sinodales por sus valiosos comentarios: Claudia, Manett y Mauricio Romero. Les agradezco por haber leído mi trabajo y por su apertura para dialogarlo y mejorarlo.
...
*"Las instituciones básicas de la sociedad, según Rawls, no deben distinguirse simplemente por ser ordenadas y eficientes: ellas deben ser, sobre todo, justas. Y si no lo son, entonces, deben ser reformadas o abolidas." (Gargarella 1999).*
...
La información no resulta contundente, pero parece señalar un incremento 1.313 puntos porcentuales en la asistencia escolar por municipio e incrementos de 6.783 alumnas y alumnos en la matrícula escolar y 0.541 aulas promedio por escuela de educación indígena en municipios del programa.
...
Para ello, se incorporarán dobles diferencias para cada año: las interacciones del programa con los años 2000 y 2005 serán los efectos placebo y las interacciones del programa con 2015 y 2020 estimarán su impacto. Idealmente, los coeficientes de los efectos placebo tendrían que ser pequeños y no significativos para que el supuesto de tendencias paralelas sea más creíble.
...
Para verificar la credibilidad del supuesto de tendencias paralelas, se incorporan estimaciones de Diferencias en Diferencias para 2000 y 2005, cuando aún no entraba en operación el Programa de Inclusión y Equidad Educativa. Por lo tanto, esta intervención placebo del programa no tendría que mostrar impacto alguno para poder suponer que los municipios del programa se pudieron haber comportado igual que los de control de 2010 a 2020.
...
Al respecto del supuesto de tendencias paralelas, no se detectan diferencias significativas para los coeficientes del efecto placebo. Además, el coeficiente de diferencias en diferencias parece no cambiar mucho en signo, magnitud y significancia al incorporar controles.
...
stargazer(modelo1, modelo2, modelo3, modelo4, modelo5,
          ...
          covariate.labels = c("Diff-in-Diff 2000", "Diff-in-Diff 2005", "Diff-in-Diff 2015", "Diff-in-Diff 2020", "Hablan lengua indígena"),
          dep.var.labels="Alfabetismo", no.space=TRUE, header=FALSE)
```
...
ggplot(diff, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  labs(x="Año", y="Alfabetismo en niñas y niños de 6 a 11 años", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
```


Asimismo, se presenta un gráfico de *event study* para visualizar el efecto del programa antes (placebo) y después de su aplicación en 2014 para cada año, incorporando intervalos de confianza al 95% para el ATT estimado de cada año.
...
```{r results="asis", echo=FALSE, warning=FALSE}
iplot(modelo_tt, xlab = "Años para el programa", ylab = "ATT estimado (intervalos de 95%)", main = "Gráfica 9. Event Study del Programa de Inclusión y\nEquidad Educativa")
```
...
Según estas estimaciones, el impacto del programa fue mayor para las niñas y niños que tenían 7 años: 5.741 puntos porcentuales entre 2010 y 2020. Los resultados para los grupos de 6 y 8 años son similares, con efectos de 4.220 y 4.494 puntos, respectivamente. Sin embargo, para las niñas y niños de 9 a 11 años, el impacto del programa parece ser que se mantuvo o no incrementó respecto del efecto de 2014-2015.

Cabe destacar que el supuesto de tendencias paralelas podría ser inválido para el caso de 6 y 7 años, toda vez que muestran coeficientes estadísticamente significativos al 5% y 1% para los placebos de 2000 o 2005. Para los grupos de 8 a 11 años, por el contrario, no parece haber diferencias significativas antes de la aplicación del programa.
...
modelo1 <- plm(alfabet ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
...
modelo3 <- plm(alfabet ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
cov3 <- vcovCR(modelo3, type="CR1", cluster=p_datos$mun)
robust_se3 <- sqrt(diag(cov3))
...
modelo4 <- plm(alfabet ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
cov4 <- vcovCR(modelo4, type="CR1", cluster=p_datos$mun)
...
modelo5 <- plm(alfabet ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
cov5 <- vcovCR(modelo5, type="CR1", cluster=p_datos$mun)
robust_se5 <- sqrt(diag(cov5))
...
modelo6 <- plm(alfabet ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
...
stargazer(modelo1, modelo2, modelo3, modelo4, modelo5, modelo6,
          ...
          covariate.labels = c("Diff-in-Diff 2000", "Diff-in-Diff 2005", "Diff-in-Diff 2015", "Diff-in-Diff 2020"),
          dep.var.labels="Alfabetismo", no.space=TRUE, header=FALSE)
```
...
ggplot(diff_6, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_text(vjust=-.5) +
  geom_text(data=subset(diff_6, trat==1 & anio == 2000), aes(label=tendencia), vjust=2.5) +
  geom_text(data=subset(diff_6, trat==1 & anio == 2005), aes(label=tendencia), vjust=2.8) +
  geom_text(data=subset(diff_6, trat==1 & anio == 2015), aes(label=tendencia), vjust=3) +
  geom_text(data=subset(diff_6, trat==1 & anio == 2020), aes(label=tendencia), vjust=4.5) +
  ggtitle("Gráfica 10. Diff-in-Diff del programa por edades:\npoblación de 6 años") +
  labs(x="Año", y="Alfabetismo en niñas y niños de 6 años", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
...
ggplot(diff_7, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_text(vjust=-.5) +
  geom_text(data=subset(diff_7, trat==1 & anio == 2000), aes(label=tendencia), vjust=1.5) +
  geom_text(data=subset(diff_7, trat==1 & anio == 2005), aes(label=tendencia), vjust=-2.8) +
  geom_text(data=subset(diff_7, trat==1 & anio == 2015), aes(label=tendencia), vjust=4.5) +
  geom_text(data=subset(diff_7, trat==1 & anio == 2020), aes(label=tendencia), vjust=5.8) +
  ggtitle("Gráfica 11. Diff-in-Diff del programa por edades:\npoblación de 7 años") +
  labs(x="Año", y="Alfabetismo en niñas y niños de 7 años", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
...
ggplot(diff_8, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_text(vjust=-.5) +
  geom_text(data=subset(diff_8, trat==1 & anio == 2000), aes(label=tendencia), vjust=2) +
  geom_text(data=subset(diff_8, trat==1 & anio == 2005), aes(label=tendencia), vjust=-2) +
  geom_text(data=subset(diff_8, trat==1 & anio == 2015), aes(label=tendencia), vjust=6.5) +
  geom_text(data=subset(diff_8, trat==1 & anio == 2020), aes(label=tendencia), vjust=7.5) +
  ggtitle("Gráfica 12. Diff-in-Diff del programa por edades:\npoblación de 8 años") +
  labs(x="Año", y="Alfabetismo en niñas y niños de 8 años", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
...
ggplot(diff_9, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_text(vjust=-.5) +
  geom_text(data=subset(diff_9, trat==1 & anio == 2000), aes(label=tendencia), vjust=-2) +
  ...
  geom_text(data=subset(diff_9, trat==1 & anio == 2020), aes(label=tendencia), vjust=7.6) +
  ggtitle("Gráfica 13. Diff-in-Diff del programa por edades:\npoblación de 9 años") +
  labs(x="Año", y="Alfabetismo en niñas y niños de 9 años", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
...
ggplot(diff_10, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_text(vjust=-.5) +
  geom_text(data=subset(diff_10, trat==1 & anio == 2000), aes(label=tendencia), vjust=-1.8) +
  geom_text(data=subset(diff_10, trat==1 & anio == 2005), aes(label=tendencia), vjust=-2) +
  geom_text(data=subset(diff_10, trat==1 & anio == 2015), aes(label=tendencia), vjust=7.2) +
  geom_text(data=subset(diff_10, trat==1 & anio == 2020), aes(label=tendencia), vjust=7) +
  ggtitle("Gráfica 14. Diff-in-Diff del programa por edades:\npoblación de 10 años") +
  labs(x="Año", y="Alfabetismo en niñas y niños de 10 años", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
...
ggplot(diff_11, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_text(vjust=-.5) +
  geom_text(data=subset(diff_11, trat==1 & anio == 2000), aes(label=tendencia), vjust=-2) +
  geom_text(data=subset(diff_11, trat==1 & anio == 2005), aes(label=tendencia), vjust=-2) +
  geom_text(data=subset(diff_11, trat==1 & anio == 2015), aes(label=tendencia), vjust=7.5) +
  geom_text(data=subset(diff_11, trat==1 & anio == 2020), aes(label=tendencia), vjust=8) +
  ggtitle("Gráfica 15. Diff-in-Diff del programa por edades:\npoblación de 11 años") +
  labs(x="Año", y="Alfabetismo en niñas y niños de 11 años", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
```


Nuevamente, se presentan los gráficos de *event study* para cada grupo de edad, con le objetivo de visualizar los coeficientes antes y después de la aplicación del programa en 2014, incluyendo los intervalos de confianza al 95%.


```{r include=FALSE}
### Event Study por edad:

# 1. Grupo de 6 años.

temp <- subset(edades, grupo == "edad_6")
temp$time_to_treat <- temp$anio - 2014
modelo_6 <- feols(alfabet ~ i(time_to_treat, trat, ref = -4) + factor(anio) + factor(mun), cluster = ~ mun, data = temp)

# 2. Grupo de 7 años.

temp <- subset(edades, grupo == "edad_7")
temp$time_to_treat <- temp$anio - 2014
modelo_7 <- feols(alfabet ~ i(time_to_treat, trat, ref = -4) + factor(anio) + factor(mun), cluster = ~ mun, data = temp)

# 3. Grupo de 8 años.

temp <- subset(edades, grupo == "edad_8")
temp$time_to_treat <- temp$anio - 2014
modelo_8 <- feols(alfabet ~ i(time_to_treat, trat, ref = -4) + factor(anio) + factor(mun), cluster = ~ mun, data = temp)

# 4. Grupo de 9 años.

temp <- subset(edades, grupo == "edad_9")
temp$time_to_treat <- temp$anio - 2014
modelo_9 <- feols(alfabet ~ i(time_to_treat, trat, ref = -4) + factor(anio) + factor(mun), cluster = ~ mun, data = temp)

# 5. Grupo de 10 años.

temp <- subset(edades, grupo == "edad_10")
temp$time_to_treat <- temp$anio - 2014
modelo_10 <- feols(alfabet ~ i(time_to_treat, trat, ref = -4) + factor(anio) + factor(mun), cluster = ~ mun, data = temp)

# 6. Grupo de 11 años.

temp <- subset(edades, grupo == "edad_11")
temp$time_to_treat <- temp$anio - 2014
modelo_11 <- feols(alfabet ~ i(time_to_treat, trat, ref = -4) + factor(anio) + factor(mun), cluster = ~ mun, data = temp)
```


```{r results="asis", echo=FALSE, warning=FALSE}
iplot(modelo_6, xlab = "Años para el programa", ylab = "ATT estimado (intervalos de 95%)", main = "Gráfica 16. Event Study por edades: población de 6 años")

iplot(modelo_7, xlab = "Años para el programa", ylab = "ATT estimado (intervalos de 95%)", main = "Gráfica 17. Event Study por edades: población de 7 años")

iplot(modelo_8, xlab = "Años para el programa", ylab = "ATT estimado (intervalos de 95%)", main = "Gráfica 18. Event Study por edades: población de 8 años")

iplot(modelo_9, xlab = "Años para el programa", ylab = "ATT estimado (intervalos de 95%)", main = "Gráfica 19. Event Study por edades: población de 9 años")

iplot(modelo_10, xlab = "Años para el programa", ylab = "ATT estimado (intervalos de 95%)", main = "Gráfica 20. Event Study por edades: población de 10 años")

iplot(modelo_11, xlab = "Años para el programa", ylab = "ATT estimado (intervalos de 95%)", main = "Gráfica 21. Event Study por edades: población de 11 años")
```
...
Según la estimación, el programa incrementó la asistencia escolar en 1.313 puntos porcentuales en los municipios cuyas escuelas de educación indígena percibieron subsidios. Aunque el coeficiente del placebo de 2000 es positivo y significativo al 5%.

La evidencia más robusta se refiere a los grupos de 9 y 11 años con efectos de 1.069 y 0.852 puntos porcentuales, respectivamente. En ambos casos, la evidencia parece señalar que se cumple el supuesto de tendencias paralelas.
...
ggplot(diff, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  ggtitle("Gráfica 22. Mecanismo causal: Diff-in-Diff sobre asistencia escolar") +
  labs(x="Año", y="Asistencia escolar de niñas y niños de 6 a 11 años", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
```
...
modelo1 <- plm(asisten ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
cov1 <- vcovCR(modelo1, type="CR1", cluster=p_datos$mun)
robust_se1 <- sqrt(diag(cov1))
...
modelo3 <- plm(asisten ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
cov3 <- vcovCR(modelo3, type="CR1", cluster=p_datos$mun)
robust_se3 <- sqrt(diag(cov3))
...
modelo4 <- plm(asisten ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
cov4 <- vcovCR(modelo4, type="CR1", cluster=p_datos$mun)
robust_se4 <- sqrt(diag(cov4))
...
modelo5 <- plm(asisten ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
cov5 <- vcovCR(modelo5, type="CR1", cluster=p_datos$mun)
robust_se5 <- sqrt(diag(cov5))
...
modelo6 <- plm(asisten ~ diff_00 + diff_05 + diff_15 + diff_20 + factor(anio), p_datos, model = "within")
cov6 <- vcovCR(modelo6, type="CR1", cluster=p_datos$mun)
robust_se6 <- sqrt(diag(cov6))
```
...
Al respecto de la matrícula escolar, se cuenta con información anual de 2000 a 2019 por escuela de educación indígena entre los municipios de control y del programa. La siguiente tabla muestra la estimación del modelo de Diferencias en Diferencias donde la primera columna es un OLS *pooled* para toda la línea de tiempo de 2000 a 2019, la segunda incluye efectos fijos y la tercera, efectos aleatorios. En todos los casos, se utilizan errores cluster por municipio.

Se observa un salto acumulado de 6.783 con significancia de apenas 10% y, al observar la línea de tiempo hacia atrás, no se encuentran diferencias significativas en los placebo. De hecho, en la gráfica se observa con mayor claridad que la curva del contrafactual parecería ser bastante "paralela" a la curva del grupo del programa hasta el salto de 2.502 estudiantes por escuela en 2014, cuando inició la operación del programa.
...
ggplot(diff, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_line(data=subset(diff, trat==1), aes(x=anio, y=tendencia), linetype="dashed") +
  geom_text(vjust=-.5) +
  ggtitle("Gráfica 23. Mecanismo causal: Diff-in-Diff sobre matrícula escolar") +
  labs(x="Año", y="Matrícula escolar en escuelas de educación indígena", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
```
...
Al respecto del número de docentes en escuelas de educación indígena, la siguiente tabla muestra los resultados del modelo. De nuevo, la primer columna es un OLS *pooled*  para toda la línea de tiempo, la segunda incluye efectos fijos y la tercera, efectos aleatorios. En todos los casos, se utilizan errores cluster por municipio.

En este caso, pareciera haber una caída importante en el grupo de control entre 2017 y 2018, mientras que el grupo del programa se mantuvo constante en el número de docentes. Derivado de esto, se observa un coeficiente de 0.397 significativo al 5% en 2018. Sin embargo, para 2019, pareciera haber una recuperación por parte del grupo de control igualando, nuevamente, a las escuelas del programa.
...
ggplot(diff, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_line(data=subset(diff, trat==1), aes(x=anio, y=tendencia), linetype="dashed") +
  geom_text(vjust=-.5) +
  ggtitle("Gráfica 24. Mecanismo causal: Diff-in-Diff sobre el número de docentes") +
  labs(x="Año", y="Número de docentes en escuelas\nde educación indígena", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
```
...
Solo se observa un coeficiente de 0.347 docentes con educación superior significativo al 5% en 2018; sin embargo, para 2019, ambos grupos se vuelven a igualar. El comportamiento es bastante similar al del caso anterior del número de docentes en general. 
...
modelo <- lm(prof_sup ~ trat + diff_00 + diff_01 + diff_02 + diff_03 + diff_04 + diff_05 + diff_06 + diff_07 + diff_08 + diff_09 + diff_10 + diff_11 + diff_12 + diff_14 + diff_15 + diff_16 + diff_17 + diff_18 + diff_19 + factor(anio), temp)
...
ggplot(diff, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_line(data=subset(diff, trat==1), aes(x=anio, y=tendencia), linetype="dashed") +
  geom_text(vjust=-.5) +
  ggtitle("Gráfica 25. Mecanismo causal: Diff-in-Diff sobre el número de docentes\ncon educación superior") +
  labs(x="Año", y="Docentes con educación superior en escuelas\nde educación indígena", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
```
...
#modelo1 <- lm(prof_sup ~ trat + diff + after_14, temp)
#cov1 <- vcovCR(modelo1, type="CR1", cluster=temp$mun)
#robust_se1 <- sqrt(diag(cov1))

# 2. Pooled OLS.

modelo2 <- lm(prof_sup ~ trat + diff_00 + diff_01 + diff_02 + diff_03 + diff_04 + diff_05 + diff_06 + diff_07 + diff_08 + diff_09 + diff_10 + diff_11 + diff_12 + diff_14 + diff_15 + diff_16 + diff_17 + diff_18 + diff_19 + factor(anio), temp)
cov2 <- vcovCR(modelo2, type="CR1", cluster=temp$mun)
robust_se2 <- sqrt(diag(cov2))
...
modelo3 <- plm(prof_sup ~ diff_00 + diff_01 + diff_02 + diff_03 + diff_04 + diff_05 + diff_06 + diff_07 + diff_08 + diff_09 + diff_10 + diff_11 + diff_12 + diff_14 + diff_15 + diff_16 + diff_17 + diff_18 + diff_19 + factor(anio), p_datos, model = "within")
cov3 <- vcovCR(modelo3, type="CR1", cluster=p_datos$mun)
robust_se3 <- sqrt(diag(cov3))

# 4. Efectos Aleatorios.

modelo4 <- plm(prof_sup ~ trat + diff_00 + diff_01 + diff_02 + diff_03 + diff_04 + diff_05 + diff_06 + diff_07 + diff_08 + diff_09 + diff_10 + diff_11 + diff_12 + diff_14 + diff_15 + diff_16 + diff_17 + diff_18 + diff_19 + factor(anio), p_datos, model = "random")
cov4 <- vcovCR(modelo4, type="CR1", cluster=p_datos$mun)
robust_se4 <- sqrt(diag(cov4))
...
stargazer(modelo2, modelo3, modelo4,
          title="Modelos de Diferencias en Diferencias",
          se=list(robust_se2, robust_se3, robust_se4),
          omit=c("trat", "anio", "dummy_14", "Constant"),
          omit.stat=c("adj.rsq", "ser"),
          font.size="tiny",
          column.sep.width="0pt",
          model.names=F,
...
En promedio, la presencia del programa está asociada con un incremento de 0.541 aulas por escuela de educación indígena. La diferencia se empieza a hacer notoria desde 2015, haciéndose cada vez mayor con el paso del tiempo. Previo a la aplicación del programa, no se observan diferencias significativas.
...
ggplot(diff, aes(x=anio, y=means, color=trat, label=means)) +
  ...
  geom_line(data=subset(diff, trat==1), aes(x=anio, y=tendencia), linetype="dashed") +
  geom_text(vjust=-.5) +
  ggtitle("Gráfica 26. Mecanismo causal: Diff-in-Diff sobre el número de aulas") +
  labs(x="Año", y="Número de aulas en escuelas de educación indígena", color="Programa", caption="La línea punteada es el contrafactual del grupo del programa") +
  xlim(2000, 2020)
```
...
Se encontró que la aplicación del programa está vinculada con un incremento 1.313 puntos porcentuales en la asistencia escolar por municipio e incrementos de 6.783 alumnas y alumnos en la matrícula escolar y 0.541 aulas por escuela de educación indígena en municipios del programa.
...
Gargarella, Roberto. (1999). *Las teorías de la justicia después de Rawls*. España: Paidos.
